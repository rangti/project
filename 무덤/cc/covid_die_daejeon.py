...
def grape():
    ...
    # 월별 합계 데이터 생성
    quarterly_sum = df[['daejeon']].apply(lambda x: number_handling(x), axis=1).resample('Q').sum()
   ...
   # total_a= [quarterly_sum[i:i+4] for i in range(0,len(quarterly_sum),4)]
    ...
    total_list = quarterly_sum['daejeon']
    ...
    #json_1 = to_json2(df)
    return render_template('graph_die.html'
